...
import opennlp.fieldspring.util.ioutil.{FileHandler, LocalFileHandler}
import opennlp.fieldspring.util.osutil.output_resource_usage
import opennlp.fieldspring.util.printutil.errprint

import opennlp.fieldspring.gridlocate._
import GridLocateDriver.Debug._

import opennlp.fieldspring.worddist.{WordDist,WordDistFactory}
import opennlp.fieldspring.worddist.WordDist.memoizer._
...
of Fieldspring.  An underlying GridLocate framework is provided
...
that code for Geolocate is split between `fieldspring.geolocate` and
`fieldspring.gridlocate`.  See also GridLocate.scala.
...
-- The main entry class is GeolocateDocumentApp.  This is hooked into
   ...
   the geolocate app and other Fieldspring apps programmatically as well
   ...
   class is in fieldspring.util.experiment.  The actual entry point is
...
-- The class GeolocateDocumentParameters holds descriptions of all of the
   ...
   Fieldspring apps.  Argument parsing is handled using
   fieldspring.util.argparser, a custom argument-parsing package built on
   top of Argot.
...
trait GeolocateDocumentTypeDriver extends GeolocateDriver with
   ...
   *
   * Seq[(java.lang.String, GridLocateDocumentStrategy[SphereCell, SphereCellGrid], scala.collection.mutable.Map[evalobj.Document,opennlp.fieldspring.geolocate.EvaluationResult])] where val evalobj: opennlp.fieldspring.geolocate.CorpusEvaluator
   *
